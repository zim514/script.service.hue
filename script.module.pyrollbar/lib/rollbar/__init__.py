...
def _build_django_request_data(request):
    ...
    request_data['headers'] = _extract_wsgi_headers(list(request.environ.items()))
...
def _build_wsgi_request_data(request):
    ...
    if 'QUERY_STRING' in request:
        ...
        request_data['GET'] = dict((k, v[0] if len(v) == 1 else v) for k, v in list(request_data['GET'].items()))

    request_data['headers'] = _extract_wsgi_headers(list(request.items()))
