...
def createHueScene(bridge):
    xbmc.log("In kodiHue createHueScene")
    ...
    if sceneName:
        ...
        if selected:
            ...
            xbmc.log("In kodiHue createHueScene. Res: {}".format(res))
            if res[0]["success"]:
...
def deleteHueScene(bridge):
    xbmc.log("In kodiHue deleteHueScene")
    ...
    if scene and confirm:
        ...
        xbmc.log("In kodiHue createHueGroup. Res: {}".format(res))
        if res[0]["success"]:
...
def _discoverNupnp():
    xbmc.log("In kodiHue discover_nupnp()")
    try:
        ...
        xbmc.log("Nupnp failed: {}".format(e))
        return None
...
def bridgeDiscover(monitor):
    ...
    while not progressBar.iscanceled() and not complete and not monitor.abortRequested():
        ...
        if connectionTest(bridgeIP):
            ...
            if bridgeUser:
                xbmc.log("User created: {}".format(bridgeUser))
                ...
                progressBar.close()
                xbmc.log("Bridge discovery complete")
                return True
            else:
                xbmc.log("User not created, received: {}".format(bridgeUser))
            ...
            xbmc.log("Bridge not found, check your bridge and network")
            monitor.waitForAbort(5)
            complete = True
            progressBar.close()

    if progressBar.iscanceled():
        xbmc.log("Bridge discovery cancelled by user")
...
def connectionTest(bridgeIP):
    xbmc.log("Connection Test IP: {}".format(bridgeIP))
...
def userTest(bridgeIP, bridgeUser):
    ...
    if zigbeechan:
        xbmc.log("Hue User Authorized. Bridge Zigbee Channel: {}".format(zigbeechan))
        return True
    return False
...
def createUser(monitor, bridgeIP, progressBar=False):
    ...
    while 'link button not pressed' in res and timeout <= 90 and not monitor.abortRequested() and not progressBar.iscanceled():
        xbmc.log("In create_user: abortRquested: {}, timer: {}".format(str(monitor.abortRequested()), timeout))
    ...
    res = req.json()
    xbmc.log("json response: {}, content: {}".format(res, req.content))

    try:
        username = res[0]['success']['username']
        return username
    except Exception:
        xbmc.log("Username exception")
        return False
...
def _getLightName(bridge, L):
    try:
        ...
        xbmc.log("getLightName Exception")
        return None
...
def selectHueLights(bridge):
    ...
    for light in hueLights:
        ...
        # xbmc.log("In selectHueGroup: {}, {}".format(hgroup,name))
        index.append(light)
    ...
    xbmc.log("lightIDs: {}".format(lightIDs))
...
def selectHueScene(bridge):
    ...
    if selected > -1:
        ...
        xbmc.log("In selectHueScene: selected: {}".format(selected))
...
def activate(bridge, kgroups, ambiGroup = None):
    ...
    """
    xbmc.log("Activating scenes")

    for g in kgroups:
        try:
            if hasattr(g, 'kgroupID'):
                xbmc.log("in sunset() g: {}, kgroupID: {}".format(g, g.kgroupID))
...
def connectBridge(monitor, silent=False):
    ...
    xbmc.log("in Connect() with settings: bridgeIP: {}, bridgeUser: {}".format(bridgeIP, bridgeUser))

    if bridgeIP and bridgeUser:
        if connectionTest(bridgeIP):
            xbmc.log("in Connect(): Bridge responding to connection test.")
        else:
            xbmc.log("in Connect(): Bridge not responding to connection test, attempt finding a new bridge IP.")
            bridgeIP = discoverBridgeIP(monitor)
            if bridgeIP:
                xbmc.log("in Connect(): New IP found: {}. Saving".format(bridgeIP))
                ADDON.setSettingString("bridgeIP", bridgeIP)

        if bridgeIP:
            xbmc.log("in Connect(): Checking User")
            if userTest(bridgeIP, bridgeUser):
                bridge = qhue.Bridge(bridgeIP, bridgeUser, timeout=QHUE_TIMEOUT)
                settings_storage['connected'] = True
                xbmc.log("Successfully connected to Hue Bridge: {}".format(bridgeIP))
                if not silent:
                    notification(_("Hue Service"), _("Hue connected"), icon=xbmcgui.NOTIFICATION_INFO, sound=False)
                return bridge
        else:
            xbmc.log("Bridge not responding")
            notification(_("Hue Service"), _("Bridge connection failed"), icon=xbmcgui.NOTIFICATION_ERROR)
            settings_storage['connected'] = False
            return None

    else:
        xbmc.log("Bridge not configured")
...
def getLightGamut(bridge, L):
    try:
        ...
        xbmc.log("Light: {}, gamut: {}".format(L, gamut))
    except Exception:
        xbmc.log("Can't get gamut for light, defaulting to Gamut C: {}".format(L))
        return "C"
...
def checkBridgeModel(bridge):
    try:
        ...
        xbmc.log("Exception: checkBridgeModel")
        return None
    if model == "BSB002":
        xbmc.log("Bridge model OK: {}".format(model))
        return True
    xbmc.log("Unsupported bridge model: {}".format(model))
...
def _get_light_states(lights, bridge):
    ...
    for L in lights:
        try:
            ...
            xbmc.log("Hue call fail: {}".format(e))
...
class HueMonitor(xbmc.Monitor):
    ...
    def onNotification(self, sender, method, data):
        if sender == ADDONID:
            xbmc.log("Notification received: method: {}, data: {}".format(method, data))

            if method == "Other.disable":
                xbmc.log("Notification received: Disable")
                cache.set("script.service.hue.service_enabled", False)

            if method == "Other.enable":
                xbmc.log("Notification received: Enable")
            ...
            if method == "Other.actions":
                ...
                xbmc.log("Action Notification: group: {}, command: {}".format(kgroupid, action))
